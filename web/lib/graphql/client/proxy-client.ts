    ...
    try {
        ...
        const query =
            typeof document === "string"
                ? document
                : ((document as TypedDocumentNode<TResult, TVariables>)?.loc
                      ?.source?.body ?? print(document));

        if (!query) {
            return {
                errors: [
                    {
                        message: "GraphQL document string is empty",
                        extensions: { code: "EMPTY_DOCUMENT" },
                    },
                ],
            };
        }
...
>(
    ...
    const result = await executeProxyGraphQL<TResult, TVariables>(
        document,
        variables,
        accessToken,
        shopDomain,
    ...
    if (result.errors?.length) {
        console.error(
            `Proxy GraphQL error for shop ${shopDomain}:`,
            result.errors,
        );
        return null;
    }
