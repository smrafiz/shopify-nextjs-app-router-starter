...
export async function GET(request: NextRequest) {
    ...
    if (!SHOP_PATTERN.test(shopParam)) {
        return NextResponse.redirect(
            new URL("/billing?error=invalid_shop", baseUrl),
        );
    }
    ...
    if (!/^\d+$/.test(chargeIdParam)) {
        return NextResponse.redirect(
            new URL("/billing?error=invalid_charge", baseUrl),
        );
    }
    ...
    if (!accessToken) {
        return NextResponse.redirect(
            new URL("/billing?error=unauthorized", request.url),
        );
    }
    ...
    try {
        ...
        if (result.errors?.length) {
            ...
            return NextResponse.redirect(
                new URL("/billing?error=shopify_error", request.url),
            );
        }
        ...
        return NextResponse.redirect(
            new URL("/billing?error=internal_error", request.url),
        );
    }
}
